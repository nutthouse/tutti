...
fn tmux_is_usable() -> bool {
    ...
    let session = format!("tutti-itest-preflight-{}", std::process::id());
...
fn zellij_is_usable() -> bool {
    if Command::new("zellij").arg("--version").output().is_err() {
        return false;
    }

    let session = format!("tutti-itest-zellij-preflight-{}", std::process::id());
    ...
    let _ = Command::new("zellij")
        ...
        .output();
    run.is_ok_and(|out| out.status.success())
}

#[test]
fn send_auto_up_wait_output_preserves_long_prompt() {
    if !tmux_is_usable() {
        // Skip if tmux is unavailable or unusable in this environment.
        return;
    }
...
#[test]
fn zellij_send_auto_up_wait_output_preserves_long_prompt() {
    if !zellij_is_usable() {
        return;
    }

    let root = unique_temp_dir("tutti-zellij-send-auto-up");
    let workspace = root.join("workspace");
    let config_home = root.join("config-home");
    fs::create_dir_all(&workspace).expect("create workspace");
    fs::create_dir_all(config_home.join(".config/tutti")).expect("create config dir");
    ...
    let workspace_name = format!("itest-zellij-send-{}", std::process::id());
    let workspace_config = format!(
        "[workspace]\n\
         name = \"{}\"\n\n\
         [workspace.auth]\n\
    ...
    let prompt = format!(
        "TUTTI_ZELLIJ_PROMPT_BEGIN_{}{}_TUTTI_ZELLIJ_PROMPT_END",
        "X".repeat(300),
        "Y".repeat(300)
    );
    let send = run_tt(
    ...
    assert!(
        normalized_stdout.contains(&normalized_prompt),
        "zellij pane output missing full long prompt\npeek:\n{}",
        peek_out
    );

    let down = run_tt(&workspace, &config_home, &["down", "etl"]);
    assert!(
        down.status.success(),
        "tt zellij down failed\nstdout:\n{}\nstderr:\n{}",
