...
- Have `plan_issue` write a structured `output_json` artifact and make `implement_code` consume that file as its primary handoff input
- Prefer artifact-driven prompt handoffs over free-form transcript scraping; use the original issue body only as secondary context
