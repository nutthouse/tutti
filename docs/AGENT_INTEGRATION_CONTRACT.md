...
6. `tt doctor --json` (or `tt doctor --strict` for CI gates) before long-running automation to validate workspace prerequisites.
...
- Daemon-backed scheduling + health endpoint:
  ...
  - Stream event types include: `agent.started`, `agent.stopped`, `agent.working`, `agent.idle`, `agent.auth_failed`, `workflow.completed`, `workflow.failed`, `handoff.generated`, `handoff.applied`
