...
- Daemon-backed scheduling + health endpoint:
  ...
  - Stream event types include: `agent.started`, `agent.stopped`, `agent.working`, `agent.idle`, `agent.auth_failed`, `agent.auth_recovered`, `agent.rate_limited`, `agent.provider_down`, `agent.provider_recovered`, `agent.recovery_attempted`, `agent.recovery_succeeded`, `agent.recovery_failed`, `workflow.started`, `workflow.completed`, `workflow.failed`, `handoff.generated`, `handoff.applied`
...
- `tt up` launch integration:
  ...
  - Codex/OpenClaw/Aider constrained mode: hard allowlist enforcement via Tutti shell-policy shims (Codex also uses `-a never -s workspace-write`).
